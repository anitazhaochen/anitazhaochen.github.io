---
title: OSI七层网络
date: 2020-07-20 11:38:06
tags: [网络, TCP, UDP, HTTP, QUIC]
category: 网络
---
...
[STP 解决方法wiki百科](https://zh.wikipedia.org/wiki/%E7%94%9F%E6%88%90%E6%A0%91%E5%8D%8F%E8%AE%AE)
...
跟踪数据包通过的路线，使用 `traceroute` ，原理：发送一个 UDP 数据包，将 TTL 设置为 1、2、3... 利用 ICMP 规则，设置 TTL，比如第一个设置为 1 ，遇到第一个路由器后就挂了，对方就会返回一个 网络差错包即超时包。依次进行，这样就拿到了所有路由器 IP，当然有的路由器压根不会回复这个 ICMP。
...
查看路由： `ip route` 或者 `route `，在有多个网关的时候可以手动为指定客户端添加路由规则，如通过客户端IP地址限制某个客户端强制走某个网口：

```shell

// 创建新的 test 路由表
# ip rule add from 192.168.1.101 table test

# ip rule ls 

0: from all lookup local 

32765: from 10.0.0.10 lookup test 
...
以上代码设置了规则，从 192.168.1.101 来的包都查 test 这个新的路由表， 后面两条命令是在 test 路由表中添加具体路由规则。
 ...
 三层算法： [OSPF Wiki百科](https://zh.wikipedia.org/zh-hans/%E5%BC%80%E6%94%BE%E5%BC%8F%E6%9C%80%E7%9F%AD%E8%B7%AF%E5%BE%84%E4%BC%98%E5%85%88) （开放式最短路径优先），某些时候可以发现多个最短路径，可以在多个路径中进行负载均衡，这常常被称为等价路由。

 基于距离矢量路由算法的 [BGP Wiki 百科](https://zh.wikipedia.org/zh/%E8%BE%B9%E7%95%8C%E7%BD%91%E5%85%B3%E5%8D%8F%E8%AE%AE) 全程边界网关协议。
...
1. 自定义链接机制，通过 ID 标识，TCP 当IP或者端口发生变化的时候，就会重连， QUIC 只要 ID 不变就不会重连。

 

2. 自定义重传机制：TCP通过自适应算法进行采样 RTT 不断调整，但是不准确，有可能发了一个1号包已超时，再次发送后，服务端并不知道 客户端 ACK 的是前一个1号包还是后一个，所以存在把时间算短了和算了长了。 QUIC，如 1号包丢了，它的下一个序号就是 2号包，而客户端通过 offset 偏移量进行确定 2 号包和 1 号包其实是同一个包，而不是通过序列号进行判断，这样 RTT 的计算时间相对准确。

3. 物阻塞的多路复用，有了自定义的链接和重传机制，就可以解决 HTTP2.0 的多路复用问题。



4. 自定义流量控制，和TCP 不同的是，QUIC ACK 是基于 offset 的，每个 offset 包来了，进入了缓存就会应答，应答后就不会重发，中间的空档会等待到来或者重发即可。窗口的起始位置为当前收到的最大的 offset，从这个 offset 到当前的 stream 所能容纳的最大缓存，是真正的窗口大小。显然这样更加精确，也不需要考虑缓存中确认了，最后由于缓存满了丢包问题。
