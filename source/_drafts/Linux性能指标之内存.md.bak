...
虚拟内存地址空间内部又被分为**内核空间和用户空间**两部分，不同CPU指令可以处理的最大长度的处理器，地址空间范围也不同，32位，最大4G，64位系统都是**定义的** 128 T，并不是说最大。分别占据内存空间的最高位和最低位，剩下的中间部分是未定义的。
...
```shell
# 提示，如果没有空硬盘不要进行写硬盘操作，否则有可能造成数据丢失
